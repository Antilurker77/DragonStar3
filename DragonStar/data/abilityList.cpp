...
static std::unordered_map<AbilityID, AbilityData> initList() {
	...
	list[AbilityID::DestructionRaid] = [] {
		AbilityData ad;

		ad.Name = "Destruction Raid";
		ad.Icon = "placeholder.png";
		ad.ID = AbilityID::DestructionRaid;
		...
		ad.Elements = { Element::Fire, Element::Lightning };
		ad.RequiredWeaponTypes = {};
		...
		ad.Cooldown = { 2500, 2500, 2500, 2500, 2500 };
		ad.MaxCharges = { 1, 1, 1, 1, 1 };
		ad.HPCost = { 0, 0, 0, 0, 0 };
		...
		ad.GetDescription = [Values = ad.Values](Actor* user, EventOptions& eventOptions, int rank) {
			...
			desc = "Fire 5 bolts of destructive energy at your target, each dealing " + value + " fire and lightning damage.";
			return desc;
		};
		ad.Execute = [Values = ad.Values](Actor* user, std::vector<Actor*>& targets, sf::Vector2i cursor, std::vector<sf::Vector2i>& targetArea, EventOptions& eventOptions, int rank) {
			if (!targets.empty()) {
	...
	list[AbilityID::DivineStrike] = [] {
		AbilityData ad;

		ad.Name = "Divine Strike";
		ad.Icon = "placeholder.png";
		ad.ID = AbilityID::DivineStrike;
		...
		ad.BonusCritPower = { 0, 0, 0, 0, 0 };
		ad.BonusDoubleStrikeChance = { 0, 0, 0, 0, 0 };
		ad.BonusHPLeech = { 25, 25, 25, 25, 25 };
		ad.BonusMPLeech = { 0, 0, 0, 0, 0 };
		ad.BonusSPLeech = { 0, 0, 0, 0, 0 };
		...
		ad.GetDescription = [Values = ad.Values](Actor* user, EventOptions& eventOptions, int rank) {
			...
			desc = "Deliver a divine strike, dealing " + dmg + "light damage. Heals for 25% of damage dealt.";
			return desc;
		};
		ad.Execute = [Values = ad.Values](Actor* user, std::vector<Actor*>& targets, sf::Vector2i cursor, std::vector<sf::Vector2i>& targetArea, EventOptions& eventOptions, int rank) {
			if (!targets.empty()) {
	...
	list[AbilityID::Freeze] = [] {
		AbilityData ad;

		ad.Name = "Freeze";
		ad.Icon = "placeholder.png";
		ad.ID = AbilityID::Freeze;
		...
		ad.Range = { 350, 350, 350, 350, 350 };
		ad.UseTime = { 100, 100, 100, 100, 100 };
		ad.Cooldown = { 4500, 4500, 4500, 4500, 4500 };
		ad.MaxCharges = { 1, 1, 1, 1, 1 };
		ad.HPCost = { 0, 0, 0, 0, 0 };
		ad.MPCost = { 12, 12, 12, 12, 12 };
		ad.SPCost = { 0, 0, 0, 0, 0 };
		...
		ad.Values = {
			{ 500, 500, 500, 500, 500 }, // Ice Damage
			{ 500, 525, 550, 575, 600 } // Stun Duration
		};
		ad.PassiveBonuses = {};
		...
		ad.GetDescription = [Values = ad.Values](Actor* user, EventOptions& eventOptions, int rank) {
			...
			desc = "Encase the target in solid ice, dealing " + value + "ice damage and stunning it for " + duration + "s.";
			return desc;
		};
		ad.Execute = [Values = ad.Values](Actor* user, std::vector<Actor*>& targets, sf::Vector2i cursor, std::vector<sf::Vector2i>& targetArea, EventOptions& eventOptions, int rank) {
			if (!targets.empty()) {
				EventResult result = Combat::SpellDamage(user, targets[0], eventOptions, Values[0][rank]);
				if (result.DidHit) {
					Combat::AddAuraStack(user, targets[0], eventOptions, AuraID::Freeze, rank);
				}
			}
		};
		ad.OnEvent = [Values = ad.Values](EventType eventType, Actor* user, Actor* target, EventOptions& eventOptions, EventResult& eventResult, int64_t& amount, Ability* ability) {
	...
	list[AbilityID::GrandHeal] = [] {
		...
		ad.GetDescription = [Values = ad.Values](Actor* user, EventOptions& eventOptions, int rank) {
			...
			desc = "Call upon grand divine energy, restoring " + amount + "HP.";
			return desc;
		};
		ad.Execute = [Values = ad.Values](Actor* user, std::vector<Actor*>& targets, sf::Vector2i cursor, std::vector<sf::Vector2i>& targetArea, EventOptions& eventOptions, int rank) {
			if (!targets.empty()) {
	...
	list[AbilityID::IcyJet] = [] {
		AbilityData ad;

		ad.Name = "Icy Jet";
		ad.Icon = "placholder.png";
		ad.ID = AbilityID::IcyJet;
		...
		ad.Elements = { Element::Ice, Element::Water };
		ad.RequiredWeaponTypes = {};
		...
		ad.GetDescription = [Values = ad.Values](Actor* user, EventOptions& eventOptions, int rank) {
			std::string desc;
			std::string value;
			std::string stunDuration = std::to_string(Values[1][rank] / 100);
			...
			desc = "Launch a freezing jet of water at your target, dealing " + value + "ice and water damage and chilling it, reducing movement speed by 25% for 5s.";
			desc += " If the target is already chilled, stun it for " + stunDuration + "s.";
			return desc;
		};
		ad.Execute = [Values = ad.Values](Actor* user, std::vector<Actor*>& targets, sf::Vector2i cursor, std::vector<sf::Vector2i>& targetArea, EventOptions& eventOptions, int rank) {
			if (!targets.empty()) {
				EventResult result = Combat::SpellDamage(user, targets[0], eventOptions, Values[0][rank]);
				if (result.DidHit) {
					if (targets[0]->HasAura(AuraID::Chilled)) {
						Combat::AddAuraStack(user, targets[0], eventOptions, AuraID::IcyJet, rank);
					}
					Combat::AddAuraStack(user, targets[0], eventOptions, AuraID::Chilled, rank);
				}
			}
	...
	list[AbilityID::Multithrust] = [] {
		AbilityData ad;

		ad.Name = "Multithrust";
		ad.Icon = "placeholder.png";
		ad.ID = AbilityID::Multithrust;
		...
		ad.GetDescription = [Values = ad.Values](Actor* user, EventOptions& eventOptions, int rank) {
			...
			desc = "Thrust three times at your target, dealing " + dmg + "weapon damage with each hit.";
			return desc;
		};
		ad.Execute = [Values = ad.Values](Actor* user, std::vector<Actor*>& targets, sf::Vector2i cursor, std::vector<sf::Vector2i>& targetArea, EventOptions& eventOptions, int rank) {
			if (!targets.empty()) {
	...
	list[AbilityID::Regenerate] = [] {
		AbilityData ad;

		ad.Name = "Regenerate";
		ad.Icon = "placeholder.png";
		ad.ID = AbilityID::Regenerate;
		...
		ad.GetDescription = [Values = ad.Values](Actor* user, EventOptions& eventOptions, int rank) {
			...
			desc = "Bathe in divine light, restoring " + amount + "HP every 1s for " + duration + "s.";
			return desc;
		};
		ad.Execute = [Values = ad.Values](Actor* user, std::vector<Actor*>& targets, sf::Vector2i cursor, std::vector<sf::Vector2i>& targetArea, EventOptions& eventOptions, int rank) {
			if (!targets.empty()) {
				Combat::AddAuraStack(user, targets[0], eventOptions, AuraID::Regenerate, rank);
			}
		};
		ad.OnEvent = [Values = ad.Values](EventType eventType, Actor* user, Actor* target, EventOptions& eventOptions, EventResult& eventResult, int64_t& amount, Ability* ability) {
	...
	list[AbilityID::Sprint] = [] {
		AbilityData ad;

		ad.Name = "Sprint";
		ad.Icon = "placeholder.png";
		ad.ID = AbilityID::Sprint;
		...
		ad.Values = {
			{ 100, 105, 110, 115, 120 }, // Movement Speed Increase
			{ 800, 800, 800, 800, 800 } // Duration
		};
		ad.PassiveBonuses = {};
		...
		ad.GetDescription = [Values = ad.Values](Actor* user, EventOptions& eventOptions, int rank) {
			...
			desc = "Increases movement speed by " + value + "% for " + duration + "s.";
			return desc;
		};
		ad.Execute = [Values = ad.Values](Actor* user, std::vector<Actor*>& targets, sf::Vector2i cursor, std::vector<sf::Vector2i>& targetArea, EventOptions& eventOptions, int rank) {
			Combat::AddAuraStack(user, user, eventOptions, AuraID::Sprint, rank);
		};
		ad.OnEvent = [Values = ad.Values](EventType eventType, Actor* user, Actor* target, EventOptions& eventOptions, EventResult& eventResult, int64_t& amount, Ability* ability) {
	...
	list[AbilityID::Waterfall] = [] {
		AbilityData ad;

		ad.Name = "Waterfall";
		ad.Icon = "placeholder.png";
		ad.ID = AbilityID::Waterfall;
		...
		ad.Range = { 350, 350, 350, 350, 350 };
		ad.UseTime = { 200, 200, 200, 200, 200 };
		ad.Cooldown = { 1400, 1400, 1400, 1400, 1400 };
		ad.MaxCharges = { 1, 1, 1, 1, 1 };
		ad.HPCost = { 0, 0, 0, 0, 0 };
		ad.MPCost = { 6, 6, 6, 6, 6 };
		ad.SPCost = { 0, 0, 0, 0, 0 };
		...
		ad.Values = {
			{ 1400, 1470, 1540, 1610, 1680 }, // Damage
			{ 250, 250, 250, 250, 250 }, // MP Cost Reduction
			{ 1000, 1000, 1000, 1000, 1000 } // Buff Duration
		};
		ad.PassiveBonuses = {};
		...
		ad.GetDescription = [Values = ad.Values](Actor* user, EventOptions& eventOptions, int rank) {
			...
			desc = "Water rushes down on your target, dealing " + value + "water damage. Reduces the MP cost of your next spell by " + buffValue + "% for " + buffDuration + "s.";
			return desc;
		};
		ad.Execute = [Values = ad.Values](Actor* user, std::vector<Actor*>& targets, sf::Vector2i cursor, std::vector<sf::Vector2i>& targetArea, EventOptions& eventOptions, int rank) {
			if (!targets.empty()) {
			...
			Combat::AddAuraStack(user, user, eventOptions, AuraID::Waterfall, rank);
		};
		ad.OnEvent = [Values = ad.Values](EventType eventType, Actor* user, Actor* target, EventOptions& eventOptions, EventResult& eventResult, int64_t& amount, Ability* ability) {
	...
	list[AbilityID::PotionLesserHealing] = [] {
		AbilityData ad;

		ad.Name = "Lesser Healing Potion";
		ad.Icon = "attack.png";
		ad.ID = AbilityID::PotionLesserHealing;
	...
	list[AbilityID::ScrollMagicMapping] = [] {
		AbilityData ad;

		ad.Name = "Magic Mapping Scroll";
		ad.Icon = "placeholder.png";
		ad.ID = AbilityID::ScrollMagicMapping;
		...
		ad.GetDescription = [Values = ad.Values](Actor* user, EventOptions& eventOptions, int rank) {
			...
			desc = "Maps the current floor.";
			return desc;
		};
		ad.Execute = [Values = ad.Values](Actor* user, std::vector<Actor*>& targets, sf::Vector2i cursor, std::vector<sf::Vector2i>& targetArea, EventOptions& eventOptions, int rank) {
			user->GetDungeonScene()->MagicMap();
			messageLog.AddMessage("The scroll reveals the surrounding area!");
		};
		ad.OnEvent = [Values = ad.Values](EventType eventType, Actor* user, Actor* target, EventOptions& eventOptions, EventResult& eventResult, int64_t& amount, Ability* ability) {
	...
	list[AbilityID::MagicWeapon] = [] {
		AbilityData ad;

		ad.Name = "Magic Weapon";
		ad.Icon = "placeholder.png";
		ad.ID = AbilityID::MagicWeapon;
		...
		ad.Values = {
			{ 3, 6, 9, 12, 15 }, // Chance
			{ 1000, 1000, 1000, 1000, 1000 }, // Haste
			{ 1000, 1000, 1000, 1000, 1000 } // Duration
		};
		ad.PassiveBonuses = {};
		...
		ad.GetDescription = [Values = ad.Values](Actor* user, EventOptions& eventOptions, int rank) {
			std::string desc;
			std::string chance = std::to_string(Values[0][rank]);
			std::string haste = std::to_string(Values[1][rank] / 10);
			std::string duration = std::to_string(Values[2][rank] / 100);

			desc = "Your direct attacks have a " + chance + "% chance to make your next spell gain " + haste + "% haste. Lasts " + duration + "s.";
		...
		ad.OnEvent = [Values = ad.Values](EventType eventType, Actor* user, Actor* target, EventOptions& eventOptions, EventResult& eventResult, int64_t& amount, Ability* ability) {
			if (eventType == EventType::Damage) {
				...
				if (std::find(cat.begin(), cat.end(), Category::Attack) != cat.end() && std::find(cat.begin(), cat.end(), Category::Direct) != cat.end()) {
					int rank = ability->GetCurrentRank();
					int chance = Values[0][rank];
					if (Random::RandomInt(1, 100) <= chance) {
						Combat::AddAuraStack(user, user, eventOptions, AuraID::MagicWeapon, rank);
					}
				}
			}
		};
	...
	list[AbilityID::Quickness] = [] {
		AbilityData ad;

		ad.Name = "Quickness";
		ad.Icon = "placeholder.png";
		ad.ID = AbilityID::Quickness;
		...
		ad.GetDescription = [Values = ad.Values](Actor* user, EventOptions& eventOptions, int rank) {
			...
			desc = "Increases your haste by " + value + "%.";
			return desc;
		};
		ad.Execute = [Values = ad.Values](Actor* user, std::vector<Actor*>& targets, sf::Vector2i cursor, std::vector<sf::Vector2i>& targetArea, EventOptions& eventOptions, int rank) {
